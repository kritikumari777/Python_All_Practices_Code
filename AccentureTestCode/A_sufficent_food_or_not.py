def RCH (r,unit,arr):
    ...
    if len(arr)<=0:
        ...
        return c+1 #because arr start from 0
    
r=int(input())
unit=int(input())
n=int(input())
arr=[int(i) for i in input().split()]
print(RCH(r,unit,arr))
...
the number of rats present in an area,
‘unit’ is the amount of food each rat consumes and
each ith element of array ‘arr’ represents
the amount of food present in ‘i+1’ house number,where 0 <= i

Note:

Return -1 if the array is null
Return 0 if the total amount of food from
all houses is not sufficient for all the rats.
...
amount of food in 1st 4 houses is sufficient for all the rats.
Thus, output is 4.'''
